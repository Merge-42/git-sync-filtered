...
def test_collect_paths_to_keep_from_args() -> None:
    result = collect_paths_to_keep(keep=("src", "docs"), keep_from_file=None)
    assert result == ["docs", "src"]


def test_collect_paths_to_keep_combines_args_and_file(tmp_path: Path) -> None:
...
def test_collect_paths_to_keep_removes_duplicates() -> None:
    result = collect_paths_to_keep(keep=("src", "docs", "src"), keep_from_file=None)

    assert result == ["docs", "src"]


def test_collect_paths_to_keep_keeps_first_occurrence() -> None:
    result = collect_paths_to_keep(
